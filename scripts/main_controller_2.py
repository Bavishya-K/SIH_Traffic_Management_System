...
from pedestrian_count_detection import get_pedestrian_counts_at_second  # Custom pedestrian count function
...
st.title("Real-Time Traffic and Pedestrian Signal Simulation")
...
def optimize_signals(in_count, out_count, emergency, waiting_times, neighbor_state, previous_lane, alpha=0.5, beta=0.7):
    ...
    for i, lane in enumerate(lane_names):
        ...
        max_consecutive = 2  # Limit consecutive activations to 2 times
    ...
    if selected_lane == previous_lane:
        ...
        consecutive_activations[selected_lane] = 1  # Reset count for the newly selected lane
...
def traffic_signal_control():
    ...
    while True:
        ...
        pedestrian_states = []  # Store pedestrian detection results
        ...
        for i, lane in enumerate(lane_names):
            ...
            # Collect pedestrian counts at target second
            left_count, right_count = get_pedestrian_counts_at_second(f"../videos/pedestrians/lane_{i+1}.mp4", target_second)
            pedestrians_detected = left_count + right_count  # You can adjust this logic as needed
            pedestrian_states.append(pedestrians_detected)
        ...
        pedestrian_signals = []
        for i, lane in enumerate(lane_names):
            if lane in signal_states['red'] and pedestrian_states[i] > 0:
                pedestrian_signals.append("Green")
            else:
                pedestrian_signals.append("Red")

        # Display Dashboard
        status_placeholder.markdown(f"### 🟢 Vehicle Green: {green_lane} ({green_duration}s) | 🔴 Vehicle Red: {', '.join(signal_states['red'])}")
        pedestrian_status = " | ".join([f"{lane}: {state}" for lane, state in zip(lane_names, pedestrian_signals)])
        st.markdown(f"### 🚶 Pedestrian Signals: {pedestrian_status}")
        ...
        traffic_df = pd.DataFrame({
            ...
            "Pedestrian Signal": pedestrian_signals
        })
